...
import (
	...
	"github.com/eclipse-xfsc/cam/service/collection/authsec"
)
...
func init() {
	log = logrus.WithField("component", "collection-authsec")
	log.Logger.Formatter = formatter.CapitalizeFormatter{Formatter: &logrus.TextFormatter{ForceColors: true}}

	cobra.OnInitialize(config.InitConfig)
}

func newCollectionAuthsecCommand() *cobra.Command {
	var cmd = &cobra.Command{
		Use:   "cam-collection-authsec",
		Short: "cam-collection-authsec launches the CAM Collection Authentication Security Module",
		Long:  "The CAM Collection Authentication Security collects authentication information of exposed Gaia-X service APIs.",
		RunE:  doCmd,
	}
...
func doCmd(_ *cobra.Command, _ []string) (err error) {
	...
	log.Info("Start Authentication Security ...")

	// create a new socket for gRPC communication
	lis, err := net.Listen("tcp", fmt.Sprintf(":%d", DefaultGrpcPort))
	if err != nil {
		log.Errorf("Authentication Security: could not listen: %v", err)
	}
	...
	var opts []service.ServiceOption[authsec.Server]
	if oAuthCred.TokenURL != "" {
		...
		opts = append(opts, authsec.WithOAuth2Authorizer(&oAuthCred))
	}

	// Create gRPC Server (srv) and register authentication security service (svc) on it
	srv := grpc.NewServer(grpcOpts...)
	svc := authsec.NewServer(opts...)
	...
	log.Infof("Starting gRPC server for Authentication Security CM on port: %d", DefaultGrpcPort)
	if err = srv.Serve(lis); err != nil {
		log.Fatalf("Authentication Security CM: failed to serve: %v", err)
	}
...
func main() {
	var cmd = newCollectionAuthsecCommand()
