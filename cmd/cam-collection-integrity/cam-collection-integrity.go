...
import (
	...
	"github.com/eclipse-xfsc/cam/service/collection/integrity"
)
...
func init() {
	log = logrus.WithField("component", "collection-integrity")
	log.Logger.Formatter = formatter.CapitalizeFormatter{Formatter: &logrus.TextFormatter{ForceColors: true}}

	cobra.OnInitialize(config.InitConfig)
}

func newCollectionIntegrityCommand() *cobra.Command {
	var cmd = &cobra.Command{
		Use:   "cam-collection-integrity",
		Short: "cam-collection-integrity launches the CAM Collection Remote Integrity Module",
		Long:  "The CAM Collection Remote Integrity collects integrity values of system components.",
		RunE:  doCmd,
	}
...
func doCmd(_ *cobra.Command, _ []string) (err error) {
	...
	log.Info("Start Remote Integrity ...")
	...
	var opts []service.ServiceOption[integrity.Server]
	if oAuthCred.TokenURL != "" {
		...
		opts = append(opts, integrity.WithOAuth2Authorizer(&oAuthCred))
	}

	// Create gRPC Server (srv) and register remote integrity service (svc) on it
	srv := grpc.NewServer(grpcOpts...)
	svc := integrity.NewServer(opts...)
	...
	log.Infof("Starting gRPC server for Remote Integrity CM on port: %d", DefaultGrpcPort)
	if err = srv.Serve(lis); err != nil {
		log.Fatalf("Remote Integrity CM: failed to serve: %v", err)
	}
...
func main() {
	var cmd = newCollectionIntegrityCommand()
